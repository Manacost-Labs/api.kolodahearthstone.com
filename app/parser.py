...
_HSGURU_STREAMER_LABEL_ATTRIBUTES = ("data-label", "data-title", "data-field")
_HSGURU_CARD_TITLE_SELECTORS = (
    ".deck-name",
    ".deck-title",
    ".archetype",
    "[data-deck-name]",
...
_HSGURU_CARD_CONTAINER_RE = re.compile(
    r"(?:deck[_-]?stats|streamer[_-]?deck|deck[_-]?card)",
    flags=re.IGNORECASE,
)
...
def _hsguru_streamer_from_text(text: str) -> str | None:
    ...
    for index, line in enumerate(lines):
        match = _HSGURU_STREAMER_LINE_RE.match(line)
        if match and match.group(1):
            return match.group(1)
        if line.rstrip(":").casefold() in _HSGURU_STREAMER_LABELS:
...
def _hsguru_streamer_from_card(card: Any, text: str) -> str | None:
    ...
    for node in card.find_all(["dt", "th", "label", "span", "div"]):
    ...
    for node in [card, *card.find_all(True)]:
        field_labels = {
            _clean_text(str(node.get(attribute) or "")).rstrip(":").casefold()
            for attribute in _HSGURU_STREAMER_LABEL_ATTRIBUTES
        }
        if not field_labels.intersection(_HSGURU_STREAMER_LABELS):
            continue
        value = _clean_text(node.get_text(" "))
        if value.casefold() not in _HSGURU_STREAMER_LABELS:
            return value
...
def _hsguru_card_has_streamer_marker(card: Any) -> bool:
    ...
    identity = " ".join(
        [
            str(card.get("id") or ""),
            *[str(value) for value in (card.get("class") or [])],
        ]
    )
    return (
        _hsguru_streamer_from_text(card.get_text("\n", strip=True)) is not None
        or _HSGURU_CARD_CONTAINER_RE.search(identity) is not None
    )
...
def _hsguru_streamer_card_roots(soup: BeautifulSoup) -> list[Any]:
    ...
    for marker in soup.select(
        "[data-clipboard-text], [data-deck-code], [data-deckstring], [value]"
    ):
        root = marker
...
def _hsguru_card_deck_code(card: Any, text: str) -> str | None:
    deck_code = _deck_code_from_cell(card)
    if deck_code:
        return deck_code
    for node in [card, *card.find_all(True)]:
        value = node.get("data-deckstring")
        if isinstance(value, str):
            deck_code = first_deck_code_from_text(value)
            if deck_code:
                return deck_code
    return first_deck_code_from_text(text)
...
def _extract_hsguru_streamer_cards(
    ...
    *,
    ...
    for card in _hsguru_streamer_card_roots(soup):
        ...
        deck_code = _hsguru_card_deck_code(
            card,
            f"{clipboard_text}\n{card_text}",
        )
        if not deck_code:
        ...
        if not title:
            title_node = card.select_one(", ".join(_HSGURU_CARD_TITLE_SELECTORS))
    ...
    if not rows:
        clipboard_values = [
            str(node.get("data-clipboard-text") or "")
            for node in soup.select("[data-clipboard-text]")
            if str(node.get("data-clipboard-text") or "").strip()
        ]
        document_text = soup.get_text("\n", strip=True)
        document_codes = {
            code
            for value in [*clipboard_values, document_text]
            if (code := first_deck_code_from_text(value))
        }
        if len(document_codes) == 1 and len(clipboard_values) == 1:
            clipboard_text = clipboard_values[0]
            deck_code = next(iter(document_codes))
            title_match = re.search(
                r"^###\s+(.+?)\s*$", clipboard_text, flags=re.MULTILINE
            )
            title = _clean_text(title_match.group(1)) if title_match else ""
            if not title:
                title_node = soup.select_one(", ".join(_HSGURU_CARD_TITLE_SELECTORS))
                title = _clean_text(title_node.get_text(" ")) if title_node else ""
            streamer = _hsguru_streamer_from_card(soup, document_text)
            deck_url = next(
                (
                    detail_url
            ...
            if title and not first_deck_code_from_text(title) and streamer:
                rows.append(
                    {
                        "Deck": title,
                        "Streamer": streamer,
