...
Для GraphQL `cards` новые клиенты должны предпочитать cursor pagination:
сохраняйте `pageInfo.nextCursor` и передавайте его в `after`. Cursor нельзя
смешивать с ненулевым `offset` или переносить между запросами с разными
фильтрами. `offset` сохранён для обратной совместимости.
