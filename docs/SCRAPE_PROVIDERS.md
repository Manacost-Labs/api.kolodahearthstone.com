...
поддерживается.
API-first и Firecrawl-primary маршруты сохраняют свой приоритет. Все 81
специализированных API-адаптера дополнительно проходят типизированный
`ResponseContract` до publish gate, а 11 HSReplay Trinkets проверяют исходный
JSON-контракт после каждого transport-канала. Для HSGuru Streamer бесплатный
direct-слой также может восстановить отсутствующий deck code с разрешённой
detail-страницы; платный запрос для этого обогащения не выполняется. Vicious
Radars требует отдельные номера отчёта и radar-выпуска, upstream-state и список
проверенных radar URL, поэтому старый официальный выпуск не маскируется под
свежий.
