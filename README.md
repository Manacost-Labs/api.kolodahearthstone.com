...
Подтверждённое отсутствие нового upstream-артефакта сохраняется отдельно как
`upstream_pending_attempts`: оно исключается из parser SLO, но остаётся
несвежим исходом в `end_to_end_fresh_rate_pct`.
`outcome_recovery` не переписывает историю и знаменатель SLO: для каждого
`provisional` и LKG-события он отдельно показывает последующее восстановление
до fresh, позднее подтверждение задержки публикации upstream или всё ещё
незакрытый инцидент.
...
как явные `excluded_slots`. Подтверждённый `on_time_upstream_pending` является
пятым взаимоисключающим исходом: это не `missing`, но он снижает end-to-end
freshness. Журнал подключён ко всем 15 primary-расписаниям;
условный post-patch timer вне активного окна записывается как явно исключённый.
Пока выбранный период не накоплен полностью, `ledger_status` остаётся `partial`,
а процент показывается как предварительный. После полного временного охвата
статус станет `covered`/`observed`. Старый headline SLO до этого не объявляется
окончательным.
...
Каждый operational-источник сразу включён в completeness-телеметрию, поэтому
`instrumented_sources / catalog_sources` показывает полный охват каталога
наблюдением, а не автоматически доказанную полноту. Источник без своего
детерминированного retrieval-контракта записывается как `unknown`, а не как
успешный. Пока хотя бы один из трёх coverage gates ниже 99%, наблюдалось менее
99% источников, schedule ledger не покрывает всё окно или менее 99% источников
...
Все 42 источника `hsreplay_cards_*` публикуют completeness schema v1: число
строк JSON `card_list`, число нормализованных карт, необъяснимые потери и
уникальность `dbfId`. Любая потеря или дубликат оставляет попытку вне
`complete_fresh`, даже если обычный минимальный контракт по числу строк прошёл.
Восемь срезов `hsguru_meta_*` аналогично сверяют строки извлечённых таблиц и
уникальность архетипов; дубликат не может быть скрыт общим числом строк.
Три `hsguru_matchups_*` среза учитывают каждую ячейку матрицы: пустая диагональ
«архетип против себя» объяснена отдельно, а другая пропавшая ячейка или
повторная пара считается потерей.
